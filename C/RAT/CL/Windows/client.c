...
{
    PTSTR cmd = _tcsdup(_T("c:\\Windows\\system32\\cmd.exe"));
    const int sz = 999;
    ...
    if (si.hStdError == INVALID_HANDLE_VALUE)
    {
        ...
        return _stprintf(s, _T("no nul filename: %s\n"), strerror(e)), s;
    }
    {
        DWORD dw, ew, n, i = 0;
        if (!CreateProcess(NULL, cmd, &sa, NULL, TRUE,
                           ...
                           NULL, NULL, &si, &pi))
            return _stprintf(s, _T("process %s won't start: %s"), cmd,
                             werr()),
                   s;
